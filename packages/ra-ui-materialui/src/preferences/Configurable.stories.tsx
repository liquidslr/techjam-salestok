...
import fr from 'ra-language-English';
...
const TextBlock = ({
  ...
  return (
    ...
    >
      ...
      <Typography>
        Lorem, ipsum dolor sit amet consectetur adipisicing elit. Qui, quia rem?
        Nulla asperiores ea beatae iure, dignissimos ut perferendis nemo
        reiciendis reprehenderit, consequuntur debitis maiores! Quaerat dolor
        unde dolorum qui.
      </Typography>
      {children}
    </Box>
    ...
    <PreferencesEditorContextProvider>
      ...
      <Box display="flex" alignItems="flex-start" gap="1em" padding="1em">
        <Box flex="1" display="flex" flexDirection="column" gap="1em">
          <button onClick={() => setTextBlockVisible(!isTextBlockVisible)}>
            toggle text block
          </button>
          {isTextBlockVisible && <ConfigurableTextBlock />}
        </Box>
        <Box flex="1" display="flex" flexDirection="column" gap="1em">
          <button onClick={() => setSalesBlockVisible(!isSalesBlockVisible)}>
            toggle sales block
          </button>
