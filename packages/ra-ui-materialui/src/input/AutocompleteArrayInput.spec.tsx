...
describe('<AutocompleteArrayInput />', () => {
  ...
  it('should extract suggestions from choices', async () => {
    ...
    userEvent.type(screen.getByLabelText('resources.posts.fields.tags'), 'a');
  ...
  it('should reset filter only when needed, even if the value is an array of objects (fixes #4454)', async () => {
    ...
    const { rerender } = render(
      <AdminContext dataProvider={testDataProvider()}>
        <SimpleForm onSubmit={jest.fn()} record={{ tags: [{ id: 't' }] }}>
          <AutocompleteArrayInput
            {...defaultProps}
            choices={[
              { id: 't', name: 'Technical' },
    ...
    rerender(
      <AdminContext dataProvider={testDataProvider()}>
        <SimpleForm onSubmit={jest.fn()} record={{ tags: [{ id: 'p' }] }}>
          <AutocompleteArrayInput
            {...defaultProps}
            choices={[
              { id: 't', name: 'Technical' },
  ...
  it('should support creation of a new choice through the onCreate event when optionText is a function', async () => {
    ...
    expect(screen.queryByText('Choice is New Kid On The Block')).not.toBeNull();
  });
