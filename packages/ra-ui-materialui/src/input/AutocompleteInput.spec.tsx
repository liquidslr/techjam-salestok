...
describe('<AutocompleteInput />', () => {
  ...
  describe('emptyText', () => {
    it('should allow to have an empty menu option text by passing a string', () => {
      ...
      fireEvent.mouseDown(screen.getByLabelText('resources.users.fields.role'));
          ...
          <SimpleForm mode="onBlur" onSubmit={jest.fn()}>
            <AutocompleteInput
              ...
              matchSuggestion={(filter, choice) => {
                ...
                if (filter === 'gugu' && choice.name === 'Angular') {
                  return true;
                }
                if (filter === 'rere' && choice.name === 'React') {
                  return true;
                }
                return false;
              }}
  ...
  it('should display options properly when labels are identical', () => {
    ...
    jest
      .spyOn(console, 'error')
      .mockImplementation((message) => (errMessage = message));
    render(
      <AdminContext dataProvider={testDataProvider()}>
        <SimpleForm onSubmit={jest.fn()} defaultValues={{ role: 2 }}>
          <AutocompleteInput
  ...
  describe('error message', () => {
    ...
    it('should be displayed if field has been touched and is invalid', async () => {
      ...
      fireEvent.blur(screen.getByLabelText('resources.users.fields.role'));
  ...
  describe('onCreate', () => {
    ...
    it('should allow the creation of a new choice', async () => {
      ...
      expect(screen.queryByDisplayValue('New Kid On The Block')).not.toBeNull();
      fireEvent.click(screen.getByLabelText('ra.action.clear_input_value'));
      fireEvent.blur(input);
      fireEvent.focus(input);
  ...
  describe('Inside <ReferenceInput>', () => {
    ...
    it('should include full record when calling onChange', async () => {
      ...
      expect(screen.getByDisplayValue('English')).not.toBeNull();
    });
  });
  ...
  it('should call the onInputChange callback', async () => {
    ...
    render(
      <AdminContext dataProvider={testDataProvider()}>
        <SimpleForm onSubmit={jest.fn()}>
          <AutocompleteInput {...defaultProps} onInputChange={onInputChange} />
        </SimpleForm>
      </AdminContext>
    );
    const input = screen.getByLabelText(
