...
describe('<TranslatableInputs />', () => {
  it('should display every input for every locale', () => {
    ...
    expect(screen.queryByDisplayValue('english description')).not.toBeNull();
    expect(screen.queryByDisplayValue('english nested field')).not.toBeNull();
  ...
  it('should display validation errors and highlight the tab which has invalid inputs', async () => {
    ...
    render(
      <AdminContext dataProvider={testDataProvider()}>
        <SimpleForm record={record} onSubmit={handleSubmit} mode="onBlur">
          <TranslatableInputs locales={['en', 'fr']} selector={<Selector />}>
    ...
    expect(tabs[1].classList.contains('RaTranslatableInputsTab-error')).toEqual(
      true
    );
  });
  ...
  it('should allow to update any input for any locale', async () => {
    ...
    fireEvent.click(screen.getByText('ra.locales.fr'));
    fireEvent.change(screen.getByDisplayValue('English nested field'), {
      target: { value: 'English nested field updated' },
    });
      ...
      expect(save).toHaveBeenCalledWith(
        {
          ...
          nested: {
            field: {
              en: 'english nested field',
              fr: 'English nested field updated',
            },
          },
        },
        expect.anything()
